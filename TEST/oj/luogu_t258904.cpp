...
void solve_fhq() {
    ...
    using Tree = OY::SortSumTreap<uint32_t, uint32_t, uint64_t>;
    auto key_mapping = [&](uint32_t i) { return keys[i]; };
    auto mapping = [&](uint32_t i) { return keys[i]; };
    Tree S(n, key_mapping, mapping);
    while (m--) {
        char op;
        uint32_t i;
        cin >> op >> i;
...
void solve_seg() {
    ...
    using Tree = OY::VectorSortSumTree<uint32_t,uint64_t>;
    Tree::_reserve(n * 2 + 500);
    auto key_mapping = [&](uint32_t i) { return ids[i]; };
    auto mapping = [&](uint32_t i) { return keys[i]; };
    Tree S(n, key_mapping, mapping, n - 1);
    while (m--) {
        char op;
        uint32_t i;
        cin >> op >> i;
