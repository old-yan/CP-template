...
void solve_bitset() {
    uint32_t n, m;
    cin >> n >> m;
    OY::VectorLazyBitset<uint32_t, true> S(n); // 223ms
    // OY::ZkwTreeBitset<true> S(n); // 683ms
    // OY::DynamicBitset S(n); // 2683ms
    while (m--) {
        char op;
        cin >> op;
        if (op == '0') {
...
void solve_seg() {
    uint32_t n, m;
    cin >> n >> m;
    OY::ASZKW::Tree<Monoid> S(n, seg{0, 0, 0, 0, 1}); // 669ms
    // OY::ASSEG::Tree<Monoid, uint32_t, 1 << 17> S(n, seg{0, 0, 0, 0, 1}); // 796ms
    while (m--) {
        char op;
        cin >> op;
        if (op == '0') {
