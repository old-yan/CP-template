...
void solve_dijk() {
    uint32_t n, m;
    cin >> n >> m;
    OY::DijkstraHeap::Graph<uint32_t> G(n, m * 2);
    // OY::DijkstraNaive::Graph<uint32_t> G(n, m * 2);
    for (uint32_t i = 0; i < m; i++) {
        uint32_t a, b, c;
        cin >> a >> b >> c;
        G.add_edge(a - 1, b - 1, c);
    ...
    using Solver = OY::DijkstraHeap::Solver<OY::DijkstraHeap::AddGroup<uint32_t>, double, false>;
    // using Solver = OY::DijkstraNaive::Solver<OY::DijkstraNaive::AddGroup<uint32_t>, double, false>;
    std::vector<Solver> sols;
    sols.reserve(n);
    for (uint32_t i = 0; i != n; i++) sols.push_back(G.calc<OY::DijkstraHeap::AddGroup<uint32_t>, double>(i));
    // for (uint32_t i = 0; i != n; i++) sols.push_back(G.calc<OY::DijkstraNaive::AddGroup<uint32_t>, double>(i));
