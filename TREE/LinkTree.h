...
namespace OY {
    namespace LinkTree {
        ...
        struct Tree {
            ...
            static size_type s_vertex_buffer[MAX_VERTEX], s_parent_buffer[MAX_VERTEX], s_use_count;
            Edge<Tp> *m_edges;
            size_type *m_vertex, *m_parent, m_root = -1, m_vertex_cnt, m_edge_cnt;
            ...
            void _tree_dp_edge(size_type a, size_type p, Tp up_dis, PreWork &&pre_work, Report &&report, AfterWork &&after_work) const {
                ...
                do_for_each_adj_edge(a, [&](size_type to, Tp dis) {
                    if (to != p) {
                        _tree_dp_edge(to, a, dis, pre_work, report, after_work);
                        report(a, to, dis);
                    }
                });
                after_work(a, up_dis);
            ...
            void resize(size_type vertex_cnt) {
                ...
                m_edges = s_buffer + (s_use_count << 1), m_vertex = s_vertex_buffer + s_use_count, m_parent = s_parent_buffer + s_use_count, m_edge_cnt = 0, s_use_count += m_vertex_cnt;
        ...
        template <typename Tp, size_type MAX_VERTEX>
        size_type Tree<Tp, MAX_VERTEX>::s_parent_buffer[MAX_VERTEX];
        template <typename Tp, size_type MAX_VERTEX>
