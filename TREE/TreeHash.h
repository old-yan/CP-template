...
namespace OY {
    struct _TreeHashTrieNode : MemoryPool<_TreeHashTrieNode> {
        ...
        _TreeHashTrieNode(uint32_t __id) : m_id(__id) {}
    };
    _TreeHashTrieNode g_treeHashTrie(0);
    uint32_t g_treeHashCount = 0;
    ...
    std::vector<uint32_t> getTreeHash(_Tree &__tree, uint32_t __root) {
        ...
        while (head < tail) {
            uint32_t i = queue[head++], childNum = __tree.m_starts[i + 1] - __tree.m_starts[i] - (i != __tree.m_root), childHash[childNum];
            ...
            for (uint32_t a : childHash) {
                ...
                if (!it->second) it->second = new _TreeHashTrieNode(-1);
                cur = it->second;
            }
