...
3. 备注
   ...
   **注意：**
   ...
   1. 本构造方法仅在树和树链剖分数据结构之间建立关联，并未实际进行运算。当树初始化完成后，仍需调用 `reset` 方法。
   2. 本数据结构只擅长维护静态的数据，和高速的查询。如果树上的点的权值是动态变化的，变化后需要及时维护，那么还是需要重链剖分。
...
3. 备注
   ...
   本构造方法不仅在树和树链剖分数据结构之间建立了关联，而且实际进行了运算，不需要再调用 `reset` 方法。

#### 3.重置

1. 数据类型

   输入参数 `_Mapping __map` ，表示从结点编号到其权值的映射。
...
3. 备注
   ...
   如果在树未初始化的时候，调用了第一个构造函数。那么等树初始化完成后，仍需要调用 `reset` 。
