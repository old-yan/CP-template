...
namespace OY {
    namespace DYNAMICBITSET {
        ...
        class Table {
            ...
            void _sanitize() {
                m_data[(m_sz - 1) / MASK_SIZE] &= _get_lead_mask(m_sz % MASK_SIZE);
            }
            size_type _next_one(size_type x) const {
                if (x + 1 >= m_sz) return -1;
                size_type last_bucket = (m_sz - 1) / MASK_SIZE, i = (x + 1) / MASK_SIZE, j = (x + 1) % MASK_SIZE;
            ...
            size_type _next_zero(size_type x) const {
                ...
                size_type last_bucket = (m_sz - 1) / MASK_SIZE, i = (x + 1) / MASK_SIZE, j = (x + 1) % MASK_SIZE;
                if (i == last_bucket) {
                    mask_type a = ~m_data[i] & _get_mask(j, m_sz % MASK_SIZE);
                ...
                size_type ctz = std::countr_zero(~m_data[i] & _get_lead_mask(m_sz % MASK_SIZE));
                return ctz == MASK_SIZE ? -1 : (i << MASK_WIDTH) + ctz;
            }
            void _left_shift(size_type x) {
                if (x) {
                    size_type last_bucket = (m_sz - 1) / MASK_SIZE, y = x / MASK_SIZE, z = x % MASK_SIZE;
                    if (z)
                        if (y) {
            ...
            void _right_shift(size_type x) {
                if (x) {
                    ...
                    if (z)
                        ...
                        m_data[last_bucket - y] = m_data[last_bucket] & _get_lead_mask(m_sz % MASK_SIZE);
                    }
        ...
        public:
            ...
            Table(const Table &rhs) : m_data(_malloc(rhs.m_sz)), m_sz(rhs.m_sz) {
                memcpy(m_data.get(), rhs.m_data.get(), ((m_sz + MASK_SIZE - 1) / MASK_SIZE) * sizeof(mask_type));
            }
            Table(Table &&rhs) noexcept : m_data(std::move(rhs.m_data)), m_sz(rhs.m_sz) {}
            void resize(size_type length) { m_data.reset(_new(m_sz = length)); }
            void regard_as(size_type length) { m_sz = length; }
            Table &operator=(const Table &rhs) {
                if (this == &rhs) return *this;
                if (bool(m_data) && m_sz < rhs.m_sz) m_data.release();
                if (!bool(m_data)) m_data.reset(_malloc(rhs.m_sz));
                m_sz = rhs.m_sz;
                memcpy(m_data.get(), rhs.m_data.get(), ((m_sz + MASK_SIZE - 1) / MASK_SIZE) * sizeof(mask_type));
                return *this;
            }
            Table &operator=(Table &&rhs) noexcept {
                if (this == &rhs) return *this;
                m_data = std::move(rhs.m_data), m_sz = rhs.m_sz;
                return *this;
            }
            size_type size() const { return m_sz; }
            void set() {
                memset(m_data.get(), -1, ((m_sz + MASK_SIZE - 1) / MASK_SIZE) * sizeof(mask_type));
                _sanitize();
            }
            ...
            void reset() {
                memset(m_data.get(), 0, ((m_sz + MASK_SIZE - 1) / MASK_SIZE) * sizeof(mask_type));
            }
            ...
            void flip() {
                size_type last_bucket = (m_sz - 1) / MASK_SIZE;
            ...
            size_type count() const { return count(0, m_sz - 1); }
            ...
            bool any() const { return any(0, m_sz - 1); }
            ...
            bool all() const { return all(0, m_sz - 1); }
            ...
            size_type kth(size_type k, size_type pos = 0) const {
                ...
                k -= a;
                for (size_type end = (m_sz + MASK_SIZE - 1) / MASK_SIZE; ++i != end;) {
            ...
            size_type first_one() const {
                size_type last_bucket = (m_sz - 1) / MASK_SIZE, i = 0;
            ...
            size_type last_one() const {
                size_type last_bucket = (m_sz - 1) / MASK_SIZE, i = last_bucket;
            ...
            size_type last_zero() const {
                size_type last_bucket = (m_sz - 1) / MASK_SIZE, i = last_bucket;
                size_type clz = std::countl_zero(~m_data[i] & _get_lead_mask(m_sz % MASK_SIZE));
                if (clz != MASK_SIZE) return (i << MASK_WIDTH) + MASK_SIZE - clz - 1;
                while (~--i && !~m_data[i]) {}
                return ~i ? (i << MASK_WIDTH) + MASK_SIZE - std::countl_zero(~m_data[i]) - 1 : -1;
            }
            ...
            void enumerate_zero(Callback &&call) const {
                ...
                size_type j = last_bucket * MASK_SIZE;
                auto mask = m_data[last_bucket] ^ _get_lead_mask(m_sz % MASK_SIZE);
                while (mask) {
                    size_type ctz = std::countr_zero(mask);
                    call(j + ctz);
                    mask ^= mask_type(1) << ctz;
            ...
            void bitor_lshift(size_type x, size_type range = -1) {
                if (!range || !x || x >= m_sz) return;
                range = std::min(range, m_sz - x);
