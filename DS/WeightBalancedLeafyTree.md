...
​	数据结构：类平衡二叉树中的重量平衡叶子树

### 二、模板功能

​	重量平衡叶子树不是典型的平衡二叉树，而是一种叶子树——即所有键值对都位于叶子结点位置。线段树也是一种这样的叶子树。在使用的时候完全不需要知道实现的细节，只需要知道对外的接口。
...
1. 数据类型
   ...
   1. `_Tag=WeightBalancedLeafySetTag` 表示树模拟 `std::set` ；
   2. `_Tag=WeightBalancedLeafyMultisetTag` 表示树模拟 `std::multiset` ；
   3. `_Tag=WeightBalancedLeafyMapTag` 表示树模拟 `std::map` ；
...
1. 数据类型：
   ...
   返回类型 `_WeightBalancedLeafyTreeNode<_Tp, _Fp, _Tag>*` ，表示元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_WeightBalancedLeafyTreeNode<_Tp, _Fp, _Tag>*` ，表示树中第一个大于等于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_WeightBalancedLeafyTreeNode<_Tp, _Fp, _Tag>*` ，表示树中第一个大于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_WeightBalancedLeafyTreeNode<_Tp, _Fp, _Tag>*` ，表示树中最后一个小于 `__key` 的元素所在的位置。
...
int main() {
    //WeightBalancedLeafyTree 的接口与替罪羊树接口一模一样
    
    //展示 set
    // std::string 类型没有默认最大值，需要手动传一个
    OY::WeightBalancedLeafyTreeContainer::Set<std::string> set("zzzzz");
    set.insert("apple");
    set.insert("cat");
    set.insert("banana");
    ...
    OY::WeightBalancedLeafyTreeContainer::Multiset<int> multiset;
    multiset.insert(3);
    multiset.insert(3);
    multiset.insert(3);
    multiset.insert(3);
    ...
    //Map没有什么好说的
    OY::WeightBalancedLeafyTreeContainer::Map<int, std::string> map;
    map.insert(1, "first");
    map.insert(2, "second");
    map.insert(3, "third");
