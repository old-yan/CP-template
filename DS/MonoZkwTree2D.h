...
namespace OY {
    namespace MONOZKW2D {
            ...
            value_type _query(const value_type *sub, size_type left, size_type right) const {
                ...
                if (left)
                    while (true) {
                        ...
                        res = Monoid::op(res, sub[(m_column_capacity + left) >> j]);
                        left = left2;
                    }
                while (left < right) {
                    size_type j = std::bit_width(left ^ right), left2 = left + (size_type(1) << (j - 1));
                    res = Monoid::op(res, sub[(m_column_capacity + left) >> (j - 1)]);
                    left = left2;
                }
                return res;
            }
        ...
        public:
            ...
            void resize(size_type row, size_type column) {
                ...
                m_sub.assign(m_row_capacity << (m_column_depth + 2), Monoid::identity());
            }
            template <typename InitMapping>
            void resize(size_type row, size_type column, InitMapping mapping) {
                ...
                for (size_type i = 0; i != m_row; i++) {
                    ...
                    for (size_type j = m_column_capacity; --j;) cur[j] = Monoid::op(cur[j * 2], cur[j * 2 + 1]);
                }
                for (size_type i = m_row_capacity; --i;) {
                    ...
                    for (size_type j = m_column_capacity; j != m_column_capacity * 2; j++) cur[j] = Monoid::op(l[j], r[j]);
                    for (size_type j = m_column_capacity; --j;) cur[j] = Monoid::op(cur[j * 2], cur[j * 2 + 1]);
                }
            }
            void modify(size_type i, size_type j, value_type val) {
                ...
                cur[k] = val;
                while ((k >>= 1) && _check_pushup(cur[k], Monoid::op(cur[k * 2], cur[k * 2 + 1]))) {}
                for (i += m_row_capacity; i >>= 1;) {
                    ...
                    size_type k = m_column_capacity + j;
                    cur[k] = Monoid::op(l[k], r[k]);
                    while ((k >>= 1) && _check_pushup(cur[k], Monoid::op(cur[k * 2], cur[k * 2 + 1]))) {}
                }
            }
            void add(size_type i, size_type j, value_type inc) { modify(i, j, Monoid::op(inc, query(i, j))); }
            ...
            value_type query(size_type row1, size_type row2, size_type column1, size_type column2) const {
                ...
                if (column1 == column2) {
                    ...
                    value_type res = Monoid::op(query(row1, column1), query(row2, column1));
                    for (row1 += m_row_capacity, row2 += m_row_capacity; row1 >> 1 != row2 >> 1; row1 >>= 1, row2 >>= 1) {
                        if (!(row1 & 1)) res = Monoid::op(res, sub[(((row1 ^ 1) * 2 + 1) << m_column_depth) + column1]);
                        if (row2 & 1) res = Monoid::op(res, sub[(((row2 ^ 1) * 2 + 1) << m_column_depth) + column1]);
                    }
                    return res;
                }
                ...
                if (row1)
                    while (true) {
                        ...
                        if (left2 > row2) break;
                        res = Monoid::op(res, _query(sub + (((m_row_capacity + row1) >> j) << (m_column_depth + 1)), column1, column2));
                        row1 = left2;
                    }
                while (row1 < row2) {
                    size_type j = std::bit_width(row1 ^ row2), left2 = row1 + (size_type(1) << (j - 1));
                    res = Monoid::op(res, _query(sub + (((m_row_capacity + row1) >> (j - 1)) << (m_column_depth + 1)), column1, column2));
                    row1 = left2;
                }
                return res;
            }
    ...
    template <typename Tp, Tp Identity, typename Operation, typename InitMapping, typename TreeType = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, Identity, Operation>>>
    ...
    using MonoMaxTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, Minimum, MONOZKW2D::ChoiceByCompare<Tp, std::less<Tp>>>>;
    template <typename Tp, Tp Maximum = std::numeric_limits<Tp>::max()>
    using MonoMinTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, Maximum, MONOZKW2D::ChoiceByCompare<Tp, std::greater<Tp>>>>;
    template <typename Tp>
    using MonoGcdTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, 0, MONOZKW2D::FpTransfer<Tp, std::gcd<Tp>>>>;
    template <typename Tp>
    using MonoLcmTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, 1, MONOZKW2D::FpTransfer<Tp, std::lcm<Tp>>>>;
    template <typename Tp, Tp OneMask = Tp(-1)>
    using MonoBitAndTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, OneMask, std::bit_and<Tp>>>;
    template <typename Tp, Tp ZeroMask = 0>
    using MonoBitOrTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, ZeroMask, std::bit_or<Tp>>>;
    template <typename Tp, Tp ZeroMask = 0>
    using MonoBitXorTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, ZeroMask, std::bit_xor<Tp>>>;
    template <typename Tp, Tp Zero = Tp()>
    using MonoSumTree2D = MONOZKW2D::Tree<MONOZKW2D::BaseMonoid<Tp, Zero, std::plus<Tp>>>;
}
