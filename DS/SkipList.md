...
​	跳表不属于平衡树，但是可以实现与平衡树类似的功能，可以封装为 `set` ，`multiset`，`map`。在使用的时候完全不需要知道实现的细节，只需要知道对外的接口。

#### 1.建立平衡树

1. 数据类型

   模板参数 `_typename _Tp` ，表示表中的键类型。

   模板参数 `_typename _Fp` ，表示表中的值类型。在 `set` 和 `multiset` 里不需要这个参数，将其设置为 `bool` 类型。

   模板参数 `typename _Compare` ，表示排序函数的类，默认为 `_Tp` 的小于号比较类。

   模板参数 `_Tag`，表示表的类型:

   1. `_Tag=SkipListSetTag` 表示表模拟 `std::set` ；
   2. `_Tag=SkipListMultisetTag` 表示表模拟 `std::multiset` ；
   3. `_Tag=SkipListMapTag` 表示表模拟 `std::map` ；
...
3. 备注
   ...
   本函数的排名以 `0` 为基准；实际等同于表中小于该键值的元素数量。
...
1. 数据类型：
   ...
   返回类型 `_SkipListNode<_Tp, _Fp, _Tag>*` ，表示元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_SkipListNode<_Tp, _Fp, _Tag>*` ，表示表中第一个大于等于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_SkipListNode<_Tp, _Fp, _Tag>*` ，表示表中第一个大于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_SkipListNode<_Tp, _Fp, _Tag>*` ，表示表中最后一个小于 `__key` 的元素所在的位置。
...
int main() {
    //SkipList 的接口与替罪羊树接口一模一样
    
    //展示 set
    // std::string 类型没有默认最大值，需要手动传一个
    OY::SkipListContainer::Set<std::string> set;
    set.insert("apple");
    set.insert("cat");
    set.insert("banana");
    ...
    OY::SkipListContainer::Multiset<int> multiset;
    multiset.insert(3);
    multiset.insert(3);
    multiset.insert(3);
    multiset.insert(3);
    ...
    //Map没有什么好说的
    OY::SkipListContainer::Map<int, std::string> map;
    map.insert(1, "first");
    map.insert(2, "second");
    map.insert(3, "third");
