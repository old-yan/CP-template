                ...
                if ((m_row = row) && (m_column = column)) {
                    ...
                    for (size_type i = 0; i != m_row; i++) {
                        ...
                        for (size_type d = 1; d != m_column_depth; d++)
                            for (size_type k = m_column * d, l = k - m_column, r = l + (1 << (d - 1)), end = m_column * (d + 1) - (1 << d) + 1; k != end;) cur[k++] = Monoid::op(cur[l++], cur[r++]);
                    }
                    for (size_type d1 = 1; d1 != m_row_depth; d1++)
                        for (size_type k1 = m_row * d1, l1 = k1 - m_row, r1 = l1 + (1 << (d1 - 1)), end1 = (m_row * (d1 + 1) - (1 << d1) + 1); k1 != end1;) {
                            ...
                            for (size_type j = 0; j != m_column; j++) cur[j] = Monoid::op(prev1[j], prev2[j]);
                            for (size_type d2 = 1; d2 != m_column_depth; d2++)
                                for (size_type k2 = m_column * d2, l2 = k2 - m_column, r2 = l2 + (1 << (d2 - 1)), end2 = m_column * (d2 + 1) - (1 << d2) + 1; k2 != end2;) cur[k2++] = Monoid::op(cur[l2++], cur[r2++]);
                        }
                }
            }
            ...
            value_type query(size_type row1, size_type row2, size_type column1, size_type column2) const {
                ...
                if (row1 == row2)
                    if (column1 == column2)
                        ...
                        return Monoid::op(sub[m_frame_size * d1 + m_stride * row1 + m_column * d2 + column1], sub[m_frame_size * d1 + m_stride * row1 + m_column * d2 + column2]);
                else if (column1 == column2)
                    return Monoid::op(sub[m_frame_size * d1 + m_stride * row1 + m_column * d2 + column1], sub[m_frame_size * d1 + m_stride * row2 + m_column * d2 + column1]);
                else
                    return Monoid::op(Monoid::op(sub[m_frame_size * d1 + m_stride * row1 + m_column * d2 + column1], sub[m_frame_size * d1 + m_stride * row1 + m_column * d2 + column2]), Monoid::op(sub[m_frame_size * d1 + m_stride * row2 + m_column * d2 + column1], sub[m_frame_size * d1 + m_stride * row2 + m_column * d2 + column2]));
            }
