...
namespace OY {
    namespace ZKW2D {
        ...
        struct Tree {
            ...
            value_type _query(const node *cur, size_type left, size_type right) const {
            ...
            void resize(size_type row, size_type column, InitMapping mapping = InitMapping()) {
                if ((m_row = row) && (m_column = column)) {
                    ...
                    m_sub.resize(m_row_capacity << (m_column_depth + 2));
                    if constexpr (!std::is_same<InitMapping, Ignore>::value) {
                        for (size_type i = 0; i != m_row; i++) {
                            node *cur = m_sub.data() + ((m_row_capacity + i) << (m_column_depth + 1));
                        ...
                        for (size_type i = m_row_capacity; --i;) {
                            node *cur = m_sub.data() + m_column_capacity * 2 * i, *l = m_sub.data() + m_column_capacity * 2 * i * 2, *r = m_sub.data() + m_column_capacity * 2 * (i * 2 + 1);
            ...
            void add(size_type i, size_type j, const modify_type &modify) {
                for (i += m_row_capacity; i; i >>= 1) {
                    node *cur = m_sub.data() + (i << (m_column_depth + 1));
            ...
            void modify(size_type i, size_type j, const value_type &val) {
                ...
                for (i += m_row_capacity; i >>= 1;) {
                    node *cur = m_sub.data() + (i << (m_column_depth + 1)), *l = m_sub.data() + ((i * 2) << (m_column_depth + 1)), *r = m_sub.data() + ((i * 2 + 1) << (m_column_depth + 1));
                    size_type k = m_column_capacity + j;
            ...
            value_type query(size_type row1, size_type row2, size_type column1, size_type column2) const {
                ...
                if (column1 == column2) {
                    ...
                    for (row1 += m_row_capacity, row2 += m_row_capacity; row1 >> 1 != row2 >> 1; row1 >>= 1, row2 >>= 1) {
                        if (!(row1 & 1)) res = node::op(res, sub[(((row1 ^ 1) * 2 + 1) << m_column_depth) + column1].get());
                        if (row2 & 1) res = node::op(res, sub[(((row2 ^ 1) * 2 + 1) << m_column_depth) + column1].get());
                    }
                    return res;
                }
                if (row1 == row2) return _query(sub + ((m_row_capacity + row1) << (m_column_depth + 1)), column1, column2);
                value_type res = node::op(_query(sub + ((m_row_capacity + row1) << (m_column_depth + 1)), column1, column2), _query(sub + ((m_row_capacity + row2) << (m_column_depth + 1)), column1, column2));
                for (row1 += m_row_capacity, row2 += m_row_capacity; row1 >> 1 != row2 >> 1; row1 >>= 1, row2 >>= 1) {
                    if (!(row1 & 1)) res = node::op(res, _query(sub + ((row1 ^ 1) << (m_column_depth + 1)), column1, column2));
                    if (row2 & 1) res = node::op(res, _query(sub + ((row2 ^ 1) << (m_column_depth + 1)), column1, column2));
                }
                return res;
            }
    ...
    template <typename Tp, typename Operation, typename InitMapping = ZKW2D::Ignore, typename TreeType = ZKW2D::Tree<ZKW2D::CustomNode<Tp, Operation>>>
    ...
    template <typename Tp, typename InitMapping = ZKW2D::Ignore, typename TreeType = ZKW2D::Tree<ZKW2D::CustomNode<Tp, const Tp &(*)(const Tp &, const Tp &)>>>
    ...
    template <typename Tp, typename InitMapping = ZKW2D::Ignore, typename TreeType = ZKW2D::Tree<ZKW2D::CustomNode<Tp, Tp (*)(Tp, Tp)>>>
    ...
    template <typename Tp>
    using ZkwSumTree2D = ZKW2D::Tree<ZKW2D::BaseNode<Tp>>;
}
