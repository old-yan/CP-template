...
namespace OY {
    namespace PerSEGCNT {
        ...
        public:
            ...
            void remove(Key key) { m_rt = _remove(m_rt, key); }
            ...
            bool any(Key key_low, Key key_high) const { return m_rt && _any(m_rt, key_low, key_high, {}); }
            bool any_bitxor(Key key_low, Key key_high, Key xor_by) const { return m_rt && _any(m_rt, key_low, key_high, BitxorFilter<Key>{xor_by}); }
        ...
        public:
            Key kth(Mapped k) const { return m_base.empty() ? m_end.kth(k)->key() : _kth(m_base.m_rt, m_end.m_rt, k, {}); }
            Key kth_bitxor(Key xor_by, Mapped k) const { return (m_base.empty() ? m_end.kth_bitxor(xor_by, k)->key() : _kth(m_base.m_rt, m_end.m_rt, k, BitxorFilter<Key>{xor_by})) ^ xor_by; }
            ...
            Mapped query(Key key_low, Key key_high) const { return m_base.empty() ? m_end.query(key_low, key_high) : _query(m_base.m_rt, m_end.m_rt, key_low, key_high, {}); }
            Mapped query_bitxor(Key key_low, Key key_high, Key xor_by) const { return m_base.empty() ? m_end.query_bitxor(key_low, key_high, xor_by) : _query(m_base.m_rt, m_end.m_rt, key_low, key_high, BitxorFilter<Key>{xor_by}); }
            bool any(Key key_low, Key key_high) const { return m_base.empty() ? m_end.any(key_low, key_high) : _any(m_base.m_rt, m_end.m_rt, key_low, key_high, {}); }
            bool any_bitxor(Key key_low, Key key_high, Key xor_by) const { return m_base.empty() ? m_end.any_bitxor(key_low, key_high, xor_by) : _any(m_base.m_rt, m_end.m_rt, key_low, key_high, BitxorFilter<Key>{xor_by}); }
            ...
            template <typename Callback>
            void enumerate(Callback &&call) const {
                _dfs(m_base.m_rt, m_end.m_rt, [&](node *p, node *q) { call(q->key(), q->m_cnt - p->m_cnt); });
            }
        };
