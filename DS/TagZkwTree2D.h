    ...
    namespace TagZKW2D {
        ...
        struct Tree {
            ...
            void _add(node *value, node *tag, size_type cur, size_type row_floor, size_type row_ceil, size_type row1, size_type row2, size_type column1, size_type column2, const value_type &modify) {
                _add_value(value + (cur << m_column_depth + 1), 1, 0, m_column - 1, column1, column2, _applied_by_row(modify, row1, row2));
            ...
            void _add(node *value, node *tag, size_type cur, size_type row_floor, size_type row_ceil, size_type row, size_type column, const value_type &modify) {
                _add_value(value + (cur << m_column_depth + 1), 0, m_column - 1, column, _applied_by_row(modify, row, row));
            ...
            value_type _query(size_type cur, size_type row_floor, size_type row_ceil, size_type row1, size_type row2, size_type column1, size_type column2, value_type &&row_modify) const {
                if (row_floor == row1 && row_ceil == row2) return _op(_applied_by_row(row_modify, row1, row2), _query_value(m_value.data() + (cur << m_column_depth + 1), 1, 0, m_column - 1, column1, column2, _default_tag()));
                row_modify = _op(row_modify, _query_tag(m_tag.data() + (cur << m_column_depth + 1), 1, 0, m_column - 1, column1, column2, _default_tag()));
                size_type mid = (row_floor + row_ceil) / 2;
                if (row2 <= mid)
            ...
            value_type _query(size_type cur, size_type row_floor, size_type row_ceil, size_type row, size_type column, value_type &&row_modify) const {
                if (row_floor == row_ceil) return _op(_applied_by_row(row_modify, row, row), _query_value(m_value.data() + (cur << m_column_depth + 1), 1, 0, m_column - 1, column, _default_tag()));
                row_modify = _op(row_modify, _query_tag(m_tag.data() + (cur << m_column_depth + 1), 1, 0, m_column - 1, column, _default_tag()));
                size_type mid = (row_floor + row_ceil) >> 1;
                if (row <= mid)
                ...
                if ((m_row = row) && (m_column = column)) {
                    ...
                    if constexpr (!std::is_same<InitMapping, Ignore>::value) {
                        ...
                        for (size_type i = m_row_capacity; --i;) {
                            node *cur = m_value.data() + m_column_capacity * 2 * i, *l = m_value.data() + m_column_capacity * 2 * i * 2, *r = m_value.data() + m_column_capacity * 2 * (i * 2 + 1);
