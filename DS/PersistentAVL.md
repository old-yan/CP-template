...
1. 数据类型
   ...
   1. `_Tag=PersistentAVLSetTag` 表示树模拟 `std::set` ；
   2. `_Tag=PersistentAVLMultisetTag` 表示树模拟 `std::multiset` ；
   3. `_Tag=PersistentAVLMapTag` 表示树模拟 `std::map` ；
...
1. 数据类型：
   ...
   返回类型 `_PersistentAVLNode<_Tp, _Fp, _Tag>*` ，表示元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_PersistentAVLNode<_Tp, _Fp, _Tag>*` ，表示树中第一个大于等于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_PersistentAVLNode<_Tp, _Fp, _Tag>*` ，表示树中第一个大于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_PersistentAVLNode<_Tp, _Fp, _Tag>*` ，表示树中最后一个小于 `__key` 的元素所在的位置。
...
int main() {
    ...
    //建立一棵 int 的树
    OY::PersistentAVLContainer::Multiset<int>T;
    T.insert(0,10);
    T.insert(1,20);
    T.insert(2,30);
    ...
    //建立一棵 string 的树
    OY::PersistentAVLContainer::Multiset<std::string>T_str;
    ...
    //建立一颗从大到小的 int 树
    OY::PersistentAVLContainer::Multiset<int,std::greater<int>>T_rev;
    T_rev.insert(0,10);
    T_rev.insert(1,20);
    T_rev.insert(2,30);
    T_rev.erase(2,20);
