...
namespace OY {
    ...
    template <typename MaskType = uint64_t, template <typename, MaskRMQ::size_type> typename InterTable = ST::Table, MaskRMQ::size_type MAX_NODE = 1 << 20, typename Iterator, typename Tp = typename std::iterator_traits<Iterator>::value_type, typename Compare = std::less<Tp>, typename TreeType = MaskRMQ::ValueTable<MaskRMQ::BaseNode<Tp, Operation>, MaskType, InterTable, MAX_NODE>>
