...
1. 数据类型
   ...
   1. `_Tag=LLRBSetTag` 表示树模拟 `std::set` ；
   2. `_Tag=LLRBMultisetTag` 表示树模拟 `std::multiset` ；
   3. `_Tag=LLRBMapTag` 表示树模拟 `std::map` ；
...
1. 数据类型：
   ...
   返回类型 `_LLRBNode<_Tp, _Fp, _Tag>*` ，表示元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_LLRBNode<_Tp, _Fp, _Tag>*` ，表示树中第一个大于等于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_LLRBNode<_Tp, _Fp, _Tag>*` ，表示树中第一个大于 `__key` 的元素所在的位置。
...
1. 数据类型：
   ...
   返回类型 `_LLRBNode<_Tp, _Fp, _Tag>*` ，表示树中最后一个小于 `__key` 的元素所在的位置。
...
int main() {
    //展示 set
    OY::LLRBContainer::Set<std::string> set;
    set.insert("apple");
    set.insert("cat");
    set.insert("banana");
    ...
    OY::LLRBContainer::Multiset<int> multiset;
    multiset.insert(3);
    multiset.insert(3);
    multiset.insert(3);
    multiset.insert(3);
    ...
    // //Map没有什么好说的
    OY::LLRBContainer::Map<int, std::string> map;
    map.insert(1, "first");
    map.insert(2, "second");
    map.insert(3, "third");
