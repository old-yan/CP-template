...
msvc14.2,C++14
*/
#ifndef __OY_HISTORYBITEX__
#define __OY_HISTORYBITEX__

#include "BIT_ex.h"

namespace OY {
    namespace HBITEX {
        using size_type = BITEX::size_type;
        template <typename Tp, template <typename> typename BufferType = BITEX::VectorBuffer>
        class Tree {
        public:
            struct info {
                Tp m_val, m_hdif;
        ...
        private:
            BITEX::Tree<info, BufferType> m_bit;
            size_type m_ver_cnt;
        public:
            Tree() = default;
    ...
    template <typename Tp, HBITEX::size_type BUFFER = 1 << 22>
    using StaticHistoryBIT_ex = HBITEX::Tree<Tp, BITEX::StaticBufferWrap<BUFFER>::template type>;
    template <typename Tp>
    using VectorHistoryBIT_ex = HBITEX::Tree<Tp>;
}
