...
1. 数据类型
   ...
   输入参数 `uint32_t __n` ，表示并查集要处理的元素个数。
...
1. 数据类型
   ...
   参数 `uint32_t __i` ，表示要寻找元素 `__i` 所在分组的首领。
...
1. 数据类型
   ...
   参数 `uint32_t __i` ，表示要查询元素 `__i` 所在分组的大小。
...
1. 数据类型
   ...
   输入参数 `uint32_t __headA`  和 `uint32_t __headB` ，分别为各自分组的首领。
...
3. 备注
   ...
   表示将元素 `__headA`  所率领的分组合并到元素 `__headB` 所率领的分组。
...
1. 数据类型
   ...
   输入参数 `uint32_t __a`  和 `uint32_t __b` 。
...
3. 备注
   ...
   表示将元素 `__a` 和元素 `__b` 所在的分组按照组的大小合并，将小组合并到大组上。

   如果 `__a` 和 `__b` 本来就在同一分组中，那么合并失败，本函数返回 `false` ；否则合并成功，返回 `true` 。
...
3. 备注
   ...
   表示将元素 `__a` 和元素 `__b` 所在的分组按照组长的 `ID` 大小合并，将 `ID` 较大的组合并到 ` ID` 较小的组上。
...
1. 数据类型：
   ...
   输入参数 `uint32_t __a`  和 `uint32_t __b` 。表示查询元素 `__a` 和元素 `__b` 是否位于同一个分组。
...
1. 数据类型
   ...
   参数 `uint32_t __i` ，表示查询元素 `__i` 是否是自己分组的首领。
