...
1. 数据类型
   ...
   参数 `uint32_t i` ，表示要查询元素 `i` 所在分组的大小。
...
1. 数据类型
   ...
   输入参数 `uint32_t head_a`  和 `uint32_t head_b` ，分别为各自分组的首领。
...
1. 数据类型：
   ...
   输入参数 `uint32_t a`  和 `uint32_t b` 。表示查询元素 `a` 和元素 `b` 是否位于同一个分组。
...
1. 数据类型
   ...
   参数 `uint32_t i` ，表示查询元素 `i` 是否是自己分组的首领。
