    ...
    struct FirstLastBiggerSmaller {
        ...
        uint32_t queryFirstSmaller(uint32_t __left, uint32_t __right, _Tp __item) const {
            ...
            if (m_minSolver.m_sub[d][__left] >= __item) {
                if (d = 32 - std::__countl_zero(__right - __left + 1 - (1 << d) - 1); d == 32 || m_minSolver.m_sub[d][__left = __right - (1 << d) + 1] >= __item)
                    return -1;
            }
            while (d--)
        ...
        uint32_t queryFirstBigger(uint32_t __left, uint32_t __right, _Tp __item) const {
            ...
            if (m_maxSolver.m_sub[d][__left] <= __item) {
                if (d = 32 - std::__countl_zero(__right - __left + 1 - (1 << d) - 1); d == 32 || m_maxSolver.m_sub[d][__left = __right - (1 << d) + 1] <= __item)
                    return -1;
            }
            while (d--)
        ...
        uint32_t queryLastSmaller(uint32_t __left, uint32_t __right, _Tp __item) const {
            ...
            else if (d = 32 - std::__countl_zero(__right - __left + 1 - (1 << d) - 1); d == 32 || m_maxSolver.m_sub[d][__left] >= __item)
                return -1;
            while (d--)
