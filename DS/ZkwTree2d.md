...
### 二、模板功能
 	...
 	二维 `Zkw` 线段树的功能和普通二维线段树的功能、接口完全一致。
...
1. 数据类型
   ...
   构造参数 `uint32_t __row` ，表示二维 `Zkw` 线段树的行范围为 `[0, __row)`。

   构造参数 `uint32_t __column` ，表示二维 `Zkw` 线段树的列范围为 `[0, __column)`。
...
3. 备注
    ...
    二维 `Zkw` 线段树和普通二维线段树相比，功能上没有差别。在空间上更紧凑，时间效率更高。
...
3. 备注
   ...
   **注意：**由于 `Zkw` 线段树会将行数和列数均补至 `2` 的整数幂，所以寻址函数在 `i >= __row` 与 `j >= __column` 时也须有相应的返回值。
...
1. 数据类型
   ...
   输入参数 `uint32_t __row` ，表示二维 `Zkw` 线段树的行范围为 `[0, __row)`。

   输入参数 `uint32_t __column` ，表示二维 `Zkw` 线段树的列范围为 `[0, __column)`。
...
3. 备注
   ...
   调用本函数会将二维`Zkw` 线段树大小改变，并将之前的合并信息重置。
...
1. 数据类型
   ...
   输入参数 `uint32_t __row` ，表示单点增值的行下标。

   输入参数 `uint32_t __column` ，表示单点增值的列下标。
...
1. 数据类型
   ...
   输入参数 `uint32_t __row` ，表示单点查询的行下标。

   输入参数 `uint32_t __column` ，表示单点查询的列下标。
...
1. 数据类型
   ...
   输入参数 `uint32_t __row1` ，表示区域查询的起始行下标。

   输入参数 `uint32_t __row2` ，表示区域查询的结束行下标。（闭区间）

   输入参数 `uint32_t __column1` ，表示区域查询的起始列下标。

   输入参数 `uint32_t __column2` ，表示区域查询的结束列下标。（闭区间）
...
int main() {
    ...
    // 除了行数、列数，还需要传递一个寻址函数
    OY::ZkwTree2d T([&](int i, int j) { return matrix[i][j]; }, 4, 5);
    // 输出[0,2]行，[1,4]列的和
    ...
    OY::ZkwTree2d<int, decltype(getmax)> T_max([&](int i, int j) { return matrix[i][j]; }, 4, 5, getmax);
    // 输出[0,2]行，[1,4]列的最大值
