...
1. 数据类型
   ...
   构造参数 `InitMapping mapping` ，表示在初始化时，从下标到值的映射函数。默认为 `Zkw::NoInit` 。接收类型可以为普通函数，函数指针，仿函数，匿名函数，泛型函数等。
...
3. 备注
   ...
   3. 定义静态常量 `unswapable` ，返回布尔值，表示聚合运算符不符合交换律。由于常见运算符均满足交换律，所以默认运算符符合交换律，将不满足交换律的少数情况另外处理。
   ...
   4. 定义静态常量 `unswapable` ，返回布尔值，表示聚合运算符不符合交换律。由于常见运算符均满足交换律，所以默认运算符符合交换律，将不满足交换律的少数情况另外处理。
   ...
   **注意：**
   ...
   构造参数中的 `mapping` 参数，入参为下标，返回值须为一个 `value_type` 对象。默认情况下， `mapping` 为 `Zkw::NoInit` 类，表示不进行初始化，比如要建立一颗空的求和线段树，由于全局变量值本身就是零，所以无需进行初始化。
...
void test_normal_tree() {
    ...
    auto tree_bit_and = OY::make_ZkwTree(A, A + 10, std::bit_and()); // 按位与
    cout << "bit_and(A[3~6]) =" << tree_bit_and.query(3, 6) << endl;

    auto tree_bit_or = OY::make_ZkwTree(A, A + 10, std::bit_or()); // 按位或
    cout << "bit_or(A[3~6])  =" << tree_bit_or.query(3, 6) << endl;

    auto tree_bit_xor = OY::make_ZkwTree(A, A + 10, std::bit_xor());
