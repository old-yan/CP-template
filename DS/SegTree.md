...
3. 备注
   ...
   **注意：**在构造时，必须一次性开够所需的空间。如果区间范围较小，那么空间分配没有问题；如果区间较大($10^9$ )，显然不可能开这么大的空间，这种场景下对线段树的操作次数往往只有 $10^5$ 的数量级甚至更少，所以使用者可以根据操作次数来预估所需空间。一般预留数百万的空间就够用了。默认的预留空间是 `4000000` 。
