...
3. 备注
   ...
   **注意：**
   ...
   正是由于 `SegTree` 不需要在一开始就初始化全部的结点，但有时 `SegTree` 需要一开始就全局初始化，所以产生了 `Complete` 参数和 `RangeMapping` 参数。尤其是 `RangeMapping` 参数，接受区间的左、右端点下标，返回这个区间的聚合值。假想有个情境下，初始时，值与下标呈一次函数关系，但是区间长度达到 `1e9` 。如果用 `ZkwTree` 显然不能维护这么大的区间；此时 `SegTree` 就游刃有余。对任何一个区间，都可以用梯形公式算出区间和，所以只初始化问题涉及到的点链即可。
...
void tricks() {
    ...
    // 显然，一开始不能初始化全部结点
    auto tree = OY::make_SegTree<int64_t, false>(1000000000u, std::plus<int64_t>(), [](int64_t l, int64_t r) {
        return (l + r) * (r - l + 1) / 2;
    });
    // 来查询一下 1~80000000 的和
