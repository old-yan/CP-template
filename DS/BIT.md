...
2. 时间复杂度
   ...
   当 `mapping` 参数为 `NoInit` 对象时，为 $O(1)$ ；否则为 $O(n)$ 。
...
3. 备注
   ...
   **注意：**
   ...
   构造参数中的 `mapping` 参数，入参为下标，返回值须为一个 `Tp` 对象。默认情况下， `mapping` 为 `BinaryIndexedTree::NoInit` 类，表示不进行初始化，比如要建立一颗空的求和树状数组，由于全局变量值本身就是零，所以无需进行初始化。
