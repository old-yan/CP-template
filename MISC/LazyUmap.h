...
msvc14.2,C++14
*/
#ifndef __OY_LAZYUMAP__
#define __OY_LAZYUMAP__
...
#define __OY_LAZYIMP__
    ...
    namespace LAZYUMAP {
        ...
        template <typename Key, typename Mapped, typename Hash = std::hash<typename Trait<Key>::type>, typename Pred = std::equal_to<typename Trait<Key>::type>>
        struct Table : TableBase<Key, Mapped> {
            ...
            using inner_table = std::unordered_map<key_type, inner_mapped_type, Hash, Pred>;
        ...
        template <typename Ostream, typename Key, typename Mapped, typename Hash, typename Pred>
        Ostream &operator<<(Ostream &out, const Table<Key, Mapped, Hash, Pred> &x) {
            out << '{';
            bool started = false;
            auto call = [&](const typename LAZYIMP::Trait<Key>::type &k, const typename Table<Key, Mapped, Hash, Pred>::mapped_type &v) {
                if (started)
                    out << ", ";
                else
                    started = true;
    ...
    namespace LAZYIMP {
        ...
        struct Trait<LAZYUMAP::Table<Key, Mapped, Hash, Pred>> {
            struct pair {
                LAZYUMAP::Table<Key, Mapped, Hash, Pred> m_val;
                typename LAZYUMAP::Table<Key, Mapped, Hash, Pred>::group::value_type m_inv;
            };
            static constexpr bool is_table = true;
        };
    }
    template <typename Key, typename Mapped, typename Hash = std::hash<typename LAZYIMP::Trait<Key>::type>, typename Pred = std::equal_to<typename LAZYIMP::Trait<Key>::type>>
    using LazyKeyAddUmap = LAZYUMAP::Table<LAZYUMAP::LazyAdd<Key>, Mapped, Hash, Pred>;
    template <typename Key, typename Mapped, typename Hash = std::hash<typename LAZYIMP::Trait<Key>::type>, typename Pred = std::equal_to<typename LAZYIMP::Trait<Key>::type>>
    using LazyMappedAddUmap = LAZYUMAP::Table<Key, LAZYUMAP::LazyAdd<Mapped>, Hash, Pred>;
    template <typename Key, typename Mapped, typename Hash = std::hash<typename LAZYIMP::Trait<Key>::type>, typename Pred = std::equal_to<typename LAZYIMP::Trait<Key>::type>>
    using LazyKeyAddMappedAddUmap = LAZYUMAP::Table<LAZYUMAP::LazyAdd<Key>, LAZYUMAP::LazyAdd<Mapped>, Hash, Pred>;
}
