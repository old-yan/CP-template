...
namespace OY {
    namespace DijkstraNaive {
        ...
        template <bool GetPath, typename Graph, size_type MAX_VERTEX>
        struct Solver {
            using Tp = typename Graph::value_type;
            using node = DistanceNode<Tp, GetPath>;
            static node s_buffer[MAX_VERTEX];
            static size_type s_use_count;
            Graph *m_graph;
            Tp m_infinite;
            node *m_distance;
            Solver(Graph *g = nullptr, const Tp &infinite = std::numeric_limits<Tp>::max() / 2) { reset(g, infinite); }
            void reset(Graph *g, const Tp &infinite = std::numeric_limits<Tp>::max() / 2) {
                if (!(m_graph = g)) return;
                m_infinite = infinite, m_distance = s_buffer + s_use_count, s_use_count += m_graph->vertex_cnt();
                for (size_type i = 0; i != m_graph->vertex_cnt(); i++) {
                    m_distance[i].m_val = m_infinite;
                    if constexpr (GetPath) m_distance[i].m_from = -1;
                }
            }
            void set_distance(size_type i, const Tp &dis = 0) { m_distance[i].m_val = dis; }
            void calc() {
                while (true) {
                    uint32_t near = -1;
                    Tp near_dis = m_infinite;
                    ...
                    if (!~near) break;
                    m_distance[near].m_visit = true;
                    m_graph->do_for_each_adj_edge(near, [&](size_type index, size_type to, const Tp &dis) {
                        Tp to_dis = near_dis + dis;
                        if (to_dis < m_distance[to].m_val) {
                            m_distance[to].m_val = to_dis;
            ...
            template <typename Callback>
            void trace_path(size_type destination, Callback &&call) const {
                size_type cur = destination;
                while (true) {
                    size_type index = m_distance[cur].m_from;
                    if (!~index) break;
                    m_graph->template do_for_edge<true>(index, [&](size_type index, size_type from, size_type to, const Tp &dis) { call(index, from, to, dis), cur = from; });
                }
            }
        };
        template <bool GetPath, typename DGraph, size_type MAX_VERTEX>
        typename Solver<GetPath, DGraph, MAX_VERTEX>::node Solver<GetPath, DGraph, MAX_VERTEX>::s_buffer[MAX_VERTEX];
        template <bool GetPath, typename DGraph, size_type MAX_VERTEX>
        size_type Solver<GetPath, DGraph, MAX_VERTEX>::s_use_count;
    }
}
