...
3. 备注
   ...
   `CapacityScaling` 算法处理的问题为费用流网络的最小费用流问题。
...
2. 时间复杂度
   ...
   $O(n\cdot m^2\cdot \log U)$ ，此处 `U` 指最大流。
...
```c++
#include "GRAPH/CapacityScaling.h"
#include "IO/FastIO.h"

int main() {
    //建图
    OY::CapacityScaling_mcmf<int, int> G(4, 5);
    //加五条边
    G.addEdge(3, 1, 300, 2);
    G.addEdge(3, 2, 200, 3);
    G.addEdge(1, 2, 200, 1);
