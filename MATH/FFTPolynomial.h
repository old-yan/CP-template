...
namespace OY {
    template <typename _Tp, uint32_t _MAXN = 1 << 22, typename _Fp = double>
    struct FFTPolynomial : std::vector<_Tp> {
        ...
        static inline struct complex {
            ...
            complex(_Tp _val) : x(_Fp(_val)), y(0) {}
        ...
        static inline uint32_t s_dftBin[_MAXN * 2], s_dftSize = 1;
        ...
        static _Iterator idft(_Iterator __iter, uint32_t __length) {
            ...
            for (uint32_t i = 0; i < __length; i++) __iter[i] = {std::round(__iter[i].x / __length), 0};
            return __iter;
        }
