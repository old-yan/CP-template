...
namespace OY {
    ...
    struct LucasTable {
        ...
        LucasTable() {
            ...
            for (mod_type i = 1; i != Tp::mod(); i++) m_factorial.push_back(m_factorial.back() * Tp(i));
            m_factorial_inv.reserve(Tp::mod());
            m_factorial_inv.push_back(m_factorial.back().inv());
            for (mod_type i = Tp::mod() - 1; i; i--) m_factorial_inv.push_back(m_factorial_inv.back() * Tp(i));
