...
1. [982. 按位与为零的三元组](https://leetcode.cn/problems/triples-with-bitwise-and-equal-to-zero/)
2. [P2398 GCD SUM](https://www.luogu.com.cn/problem/P2398)
3. [P4717 【模板】快速莫比乌斯/沃尔什变换 (FMT/FWT)](https://www.luogu.com.cn/problem/P4717)
...
​	本模板包含了快速按位与变换、快速按位或变换、快速异或变换、快速最值变换、快速 `gcd/lcm` 变换等基础变换；同时提供了一个快速转换器，可以自由添加变换关系。

​	快速按位与变换、快速按位或变换通常被称为快速莫比乌斯变换；快速异或变换通常被称为快速沃尔什变换。

**注意：**

​	何为卷积变换？名字虽然高端，解释起来却不难。

​	假设 `a` 为一个长度为 `n` 的数组， `b` 为一个长度为 `n` 的数组。令$c_k = \sum\limits_{op(i,j)==k}a_i\cdot b_j$ ，其中 `op` 为一种二元运算。由此形成的新的数组 `c` ，成为 `a` 和 `b` 的卷积结果。不同的卷积，改变的只有 `op` 指代的具体运算。

​	显然，卷积的暴力计算需要两重循环， $O(n^2)$ 的时间复杂度。然而，对于一些特定的卷积，往往可以通过如下形式进行加速运算：

```
transform<true>(a.begin(), a.end());
transform<true>(b.begin(), b.end());
for (auto i = 0; i < n ; i++) c[i] = a[i] * b[i];
transform<false>(c.begin(), c.end());
```

​	 `transform` 操作的复杂度往往是低于 $O(n^2)$ 的；这样就实现了卷积运算的加速。
