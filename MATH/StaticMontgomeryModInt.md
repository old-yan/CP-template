...
3. 备注
   ...
   **注意：**本模板和另一个蒙哥马利连乘自取模整数模板的区别是，本模板的模数在编译期是已知的，所以在运行期不可以改变。在计算效率上，本模板比较快，所以如果模数在编译期就已知，最好使用本模板。
...
```c++
#include "IO/FastIO.h"
#include "MATH/StaticMontgomeryModInt.h"

int main() {
    //可以给类型起个名字，方便识读
    using mint_97 = OY::StaticMontgomeryModInt32<97, true>;
    ...
    for (int i = 0; i < 95; i++) A[i] = mint_97::raw(mint_97::mg.init(i + 1));
    //另开一个数组，从前往后累乘
    mint_97 B[95];
    B[0] = 1;
    ...
    using mint_233 = OY::StaticMontgomeryModInt64<233, false>;
    //求 2 的 8 次方
